...
<div class="panel panel-default">
	...
	<th class="center">Average</th>active
	-->
	</tr>
	</thead>																	
	<tbody>
	@if( count($projects)  > 0 )
		...
		<?php if($val->active == 0) { ?></br><span class="label label-warning">Draft</span><?php } elseif( $val->active == 1 ) { ?></br><span class="label label-success">Approved</span><?php } elseif( $val->active == 2 ) { ?></br><span class="label label-danger">Waiting for approval</span><?php } elseif( $val->active == 3 ) { ?></br><span class="label label-warning">Rejected</span><?php } ?>
		</td>
		...
		<?php
		if($val->approval_live != "0000-00-00") { $pdt=time();$dt=strtotime($val->approval_live); if($pdt > $dt ) { $diff=($pdt - $dt); $day=floor($diff/(60*60*24)); } else { $day= "Exp"; } } else { $day="NA"; }
		?>
		<td class="dl"><?php echo $day; ?></td> 
		<td class="dl"><?php echo date("d-m-Y", strtotime($val->created_at)); ?></td> 
		<td class="dl"><?php if($val->approval_live != "0000-00-00") { echo date("d-m-Y", strtotime($val->approval_live)); } else{ echo "NA"; } ?></td>
 
		<td class="dl">
			<?php if( $val->featured == 1 ) { ?>
			<a class="label label-sm label-featured staffpick-toggle-status" href="return confirm('Are you sure?');"   data-user="<?php echo $val->id; ?>" data-status="0" id="activefspn_<?php echo $val->id; ?>"  >Unset</a>
			<?php } else { ?>
			<a class="label label-sm label-unfeatured staffpick-toggle-status"   href="return confirm('Are you sure?');"  data-user="<?php echo $val->id; ?>" data-status="1" id="inactivefspn_<?php echo $val->id; ?>"   >Set</a>
			<?php } ?> 
		</td>
		...
		<?php if( $val->active == 0 ) { ?> 
 		...
 		<?php } else if( $val->active == 1 ) { ?><a class="label label-sm label-success" href="javascript:changestatus( <?php echo $val->id; ?>, '0' )" id="activespn_<?php echo $val->id; ?>"  >Approved</a><a class="label label-sm label-danger"   href="javascript:changestatus( <?php echo $val->id; ?>, '1' )"  id="inactivespn_<?php echo $val->id; ?>" style="display:none;"  >Pending</a><?php } else if( $val->active == 2 ) { ?> <a class="label label-sm label-danger"   href="javascript:changestatus( <?php echo $val->id; ?>, '1' )"  id="inactivespn_<?php echo $val->id; ?>"   >Waiting for approval</a><a class="label label-sm label-success" href="javascript:changestatus( <?php echo $val->id; ?>, '0' )" id="activespn_<?php echo $val->id; ?>"  style="display:none;"  >Approved</a> <?php } else { ?><span class="label label-warning">Rejected</span> <?php } ?>
		...
		<li role="presentation">
		<a role="menuitem" tabindex="-1" href="/admin/project/setaslive/{{ $val->id }}">
...
@section('scripts')
	...
	<script src="/js/ui-customs.js"></script>
		<script>
			...
			{
					var checkq = confirm("Are you sure !");
