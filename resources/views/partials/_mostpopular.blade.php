  <div class="container">
    <div class="row">
      <h3 class=" wow fadeInUp">Most popular<br>
        <span>
		sed do eiusmod tempor incididunt ut labore iusmod tempor incididunt</span> <a href="{{ URL::to('project/lists/popular')}}" class="viewAll">View all Projects</a> </h3>      
      <!-- box area open -->
	...
	@if ( count($_featuredProducts) > 0 ) 		
        ...
        <div class="mstPoprlBx wow @if($i == 0 ) fadeInLeft @elseif ($i == 1) fadeInUp @elseif ($i == 2) fadeInRight @endif ">
		 ...
		 @if($project->status == 3) <span class="mTagBx"><img src="/images/frontend/tag-icon.png" alt="" border="0"></span> @endif
          ...
          <h6><a href="{{ URL::to('project/'.$project->id . '/' . $project->slug)}}"  title="{{ $project->name }}" >{!! str_limit($project->name, $limit = 30, $end = '...') !!}</a><span>by {{ $project->posted_by }}</span></h6>
