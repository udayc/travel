...
<div class="innrPgArea registration">
  <div class="container">
	...
	<td class="hidden-xs">{{ $val->message}} on <code>{{ $val->created_at}}</code><p>
	<a href="{{ URL::to('/user/profile/'. $val->user_id)}}" target="_blank" title="{{ $val->user()->first()->name }}"><img src="/images/avtar-image/resize/{{ \App\Profile::where('user_id' , $val->user()->first()->id )->first()->user_avtar }}" width="50" width="50"/></a>
	</p>							
	</td>
	<td><code>{{ $val->action}}</code></td>
